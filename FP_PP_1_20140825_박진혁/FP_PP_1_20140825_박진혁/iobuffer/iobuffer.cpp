...
{	
	// move pointer(stream) from beg to "beg + recref"
	stream . seekg (recref, ios::beg);
	// something wrong happend about moving pointer to recref
...
// write specified record
{
	...
	// pointer moving got something wrong
	if ((int)stream . tellp () != recref) return -1;
	// write something at the point of stream
	return Write (stream);
}
